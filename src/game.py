...
class Game:
    ...
    def get_num_dice(self, player):
        ...
        if isinstance(player, Computer):
            ...
            num_dice = (
                self.dice.roll()
            )  # Generate a random number of dice for the computer
    ...
    def change_name(self, players, scoreboard):
        ...
        if players:  # Check if there are previous players
            ...
            while True:
                try:
                    choice = int(
                        input("Enter the number corresponding to the player: ")
                    )
                    if 1 <= choice <= len(players):
                        ...
                        if new_name == old_name:
                            ...
                            for (
                                name,
                                games_played,
                            ) in self.scoreboard.games_played.items():
                                if name == old_name:
                                    self.scoreboard.games_played[new_name] = (
                                        games_played
                                    )
